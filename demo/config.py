...
#   2. the production artifact in datawarehouse-ai-analysis
#   3. a demo-trained artifact (scripts/train_item2vec.py) as fallback
_PROD_ITEM2VEC = os.path.join(ANALYSIS_DIR, 'career_path_transformer_20260528_074025.bin')
_DEMO_ITEM2VEC = os.path.abspath(os.path.join(
    os.path.dirname(os.path.abspath(__file__)), '..', 'artifacts', 'item2vec.bin'))
ITEM2VEC_BIN = os.environ.get('CPT_ITEM2VEC_BIN') or (
    _PROD_ITEM2VEC if os.path.exists(_PROD_ITEM2VEC) else _DEMO_ITEM2VEC
)
